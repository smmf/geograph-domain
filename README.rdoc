...
2. Generate the domain model using the following comand (you can find the dml in src/common/dml/geograph.dml):
    mvn clean package
3. Copy the new domain model from the /target directory, inside the Geograph folder lib/cloud_tm/jars and the dml inside lib/ispn/conf

*Note*  You may need to copy the DML jars into geograph by executing the following commands:

$ find ~/.m2/repository/pt/ist -name \*.jar | grep 2.0-cloudtm > libs_paths
$ for i in `cat libs_paths`; do cp $i PATH_TO_GEPGRAPH/lib/ispn/jars; done
